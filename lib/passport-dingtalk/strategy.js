...
function Strategy(options, verify) {
    ...
    preloadToken(options.clientID,options.clientSecret).bind(this).then(function(err,access_token) {
        var self = this;
        if (err) {
        ...
        this._oauth2.getAuthorizeUrl = function(params) {
            var params= params || {};
            var options = {
                appid: self._clientId,
                redirect_uri: params['redirect_uri'],
                response_type: params['response_type'],
                scope: params['scope'],
                state: params['state']
            }
            return self._baseSite + self._authorizeUrl + "?" + querystring.stringify(options)+'#ding_redirect';
        }
        ...
        this._oauth2.getOAuthAccessToken = function(code, params, callback) {
            ...
            self._request("POST", self._persistentCodeUrl() + "?access_token=" + self.access_token , post_headers, post_data, null, function(error, data, response) {
                if( error ) {
                    callback(error);
                } else {
                    var results;
                    ...
                    if (results['errcode'] === 0 && results['errmsg'] === 'ok') {
                        ...
                        self._request("POST",self._snsTokenUrl + "?access_token=" + self.access_token, post_headers, post_data, null, function(error,data,response) {
                            if( error ) {
                                callback(error);
                            } else {
                                var results;
